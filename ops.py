...
def conv3d(input_, output_dim, 
           k_h=4, k_w=4, k_l=4, d_h=2, d_w=2, d_l=2, stddev=0.02,
           name="conv3d"):
    with tf.variable_scope(name):
        w = tf.get_variable('w', [k_h, k_w, k_l, input_.get_shape()[-1], output_dim],
                            initializer=tf.truncated_normal_initializer(stddev=stddev))
        conv = tf.nn.conv3d(input_, w, strides=[1, d_h, d_w, d_l, 1], padding='SAME')
...
def deconv3d(input_, output_shape,
    ...
    with tf.variable_scope(name):
        # filter : [height, width, output_channels, in_channels]
        w = tf.get_variable('w', [k_h, k_w, k_l, output_shape[-1], input_.get_shape()[-1]],
                            initializer=tf.random_normal_initializer(stddev=stddev))
        
        try:
            deconv = tf.nn.conv3d_transpose(input_, w, output_shape=output_shape,
                                strides=[1, d_h, d_w, d_l, 1])
